...
/// <summary>
/// MAVLink SET_POSITION_TARGET_GLOBAL_INT message (ID 86).
/// Sets a desired vehicle position, velocity, and/or acceleration in a global coordinate system (WGS84).
/// </summary>
public readonly record struct SetPositionTargetGlobalInt : IMavlinkMessage
{
    /// <summary>Payload size in bytes.</summary>
    public const int PayloadSize = 53;

    /// <summary>Timestamp (time since system boot). The rationale for the timestamp in the setpoint is to allow the system to compensate for the transport delay of the setpoint.</summary>
    ...
    /// <summary>Altitude in meters in AMSL altitude(not WGS84 if absolute or relative) ABOVE MSL.</summary>
    ...
    /// <inheritdoc/>
    public uint MessageId => 86;

    /// <inheritdoc/>
    public byte CrcExtra => 5;
    ...
    /// <summary>Deserializes a <see cref="SetPositionTargetGlobalInt"/> from a raw payload span.</summary>
    public static SetPositionTargetGlobalInt Deserialize(ReadOnlySpan<byte> buffer)
    {
