...
- `protos/` — Synced local cache of shared `.proto` files authored in `buf.build/resq-software/resq-proto`.
...
- **Protos** are synced from `buf.build/resq-software/resq-proto` before local builds and then compiled via `protoc`; regenerate with `bash scripts/sync-protos.sh && dotnet build ResQ.Protocols/ResQ.Protocols.csproj`.
