...
namespace ResQ.Mavlink.Gateway.Tests.Infrastructure;

/// <summary>
/// A controllable fake <see cref="IMavlinkTransport"/> for use in unit and integration tests.
/// Packets can be injected into the receive stream via <see cref="InjectPacket"/>, and all
/// packets sent via <see cref="SendAsync"/> are collected in <see cref="SentPackets"/>.
/// </summary>
internal sealed class TestTransport : IMavlinkTransport
{
    ...
    /// <summary>Gets all packets that have been sent via <see cref="SendAsync"/>.</summary>
    public IReadOnlyList<MavlinkPacket> SentPackets => _sent;

    /// <inheritdoc/>
    public TransportState State { get; private set; } = TransportState.Connected;

    /// <summary>Injects a packet into this transport's receive stream.</summary>
